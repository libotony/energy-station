...
// Any one can be a sponsor of a contract, just by calling sponsor function, and also the sponsor identity can be cancelled by calling unsponsor function.
...
// If the current sponsor is out of energy, master can select sponsor from other sponsors candidates by calling selectSponsor function.
...
interface Prototype {
    ...
    /// @notice check if 'sponsorAddress' is the sponsor of contract 'self'.
